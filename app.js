...
function viewHome() {
  ...
  const info = el("div", { class: "card" }, [
    el("h3", {}, ["Daily Tip"]),
    el("p", {}, [
      "Use keyboard: 1–6 = select option, N/P = next/prev, R = mark, S = submit.",
    ]),
  ]);
...
function startTest(settings) {
  ...
  APP.state = {
    ...
    items: pool.map((q) => ({
      ...
      answerIndex: null,
      correctIndex: q.answerIndex, // careful: options are shuffled; map later
...
function viewAbout() {
  $("#app").appendChild(
    el("div", { class: "card" }, [
      ...
      el("p", { class: "small" }, [
        "Weekly update: replace data/questions.latest.json with your new batch.",
      ]),
    ])
  );
}
